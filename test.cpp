...
int main(int argc, char *argv[])
{
	...
	for( int i = 0 ; i < ordered_vertices.size() ; i++ )
	{
		...
		// cout<<"Number of right neighbours for "<<ordered_vertices[ i ]<<" is "<<neighbourR[ ordered_vertices[ i ] - 1 ].size()<<endl;
	} 
	...
	for( int i = ordered_vertices.size() - 1 ; i > 0   ; i-- )
	{
		...
		for( int j = i - 1 ; j >= 0 ; j-- )
		{
			// cout<<"Value of edge from "<<ordered_vertices[ i ]<<" to "<<ordered_vertices[ j ]<<" is "<<graph[ ordered_vertices[ i ] - 1 ][ ordered_vertices[ j ] - 1 ]<<endl;
			if( graph[ ordered_vertices[ i ] - 1 ][ ordered_vertices[ j ] - 1 ] != infy )
			{
				neighbourL[ ordered_vertices[ i ] - 1 ].push_back( ordered_vertices[ j ] - 1 );
				// cout<<"Edge pushed back for left neighbour "<<endl;
			}
		}
		// cout<<"Number of left neighbours for "<<ordered_vertices[ i ]<<" is "<<neighbourL[ ordered_vertices[ i ] - 1 ].size()<<endl;
	} 

	// cout<<"After the shit"<<endl;
